...
class BaseBag {
    struct BagNode {
        BaseItem* item;
        BagNode* next;
        BagNode(BaseItem* inp, BagNode* nextptr = nullptr)
        {
            item = inp;
            next = nextptr;
        }
...
public:
    ...
    virtual void ScanForPhoenixAndUse(BaseKnight* knight);
};
...
protected:
    ...
    int baseDamage;
public:
    BaseOpponent(int lv, int num, int gil_num, int bd)
    {
        level = lv;
        type = num;
        gil = gil_num;
...
struct Config {
    ...
    Config()
    {
        ...
        hasHair = false;
        hasSword = false;
        hadDefeatedOmega = false;
        hadDefeatedHades = false;
    } 
};
